...
class CargoPathSwitch(Model):
    ...
    pathPool = ConditionalField(ModelField(CargoPath), arg_fields='cargoPathIndex', condition_func=ne(0))
...
class CargoTraffic(Model):
    ...
    beltPool = ArrayField(ModelField(BeltComponent), length_field='beltCursor', length_func=decr())
    beltRecycle = ArrayField(Int32Field, length_field='beltRecycleCursor')
    splitterPool = ArrayField(ModelField(SplitterComponent), length_field='splitterCursor', length_func=decr())
    splitterRecycle = ArrayField(Int32Field, length_field='splitterRecycleCursor')
    pathPool = ArrayField(ModelField(CargoPathSwitch), length_field='pathCursor', length_func=decr())
