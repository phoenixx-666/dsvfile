...
class StationComponent(Model):
    ...
    unused1 = Int32Field()
    workDroneDatas = ArrayField(ModelField(DroneData), length_field='workDroneCount')
    workDroneOrders = ArrayField(ModelField(LocalLogisticOrder), length_field='workDroneCount')
    idleShipCount = Int32Field()
    workShipCount = Int32Field()
    idleShipIndices = UInt64Field()
    workShipIndices = UInt64Field()
    unused2 = Int32Field()
    workShipDatas = ArrayField(ModelField(ShipData), length_field='workShipCount')
    workShipOrders = ArrayField(ModelField(RemoteLogisticOrder), length_field='workShipCount')
    numStorage = ArrayField(ModelField(StationStore))
    slots = ConditionalField(ArrayField(ModelField(Slot)), arg_fields='version', condition_func=ge(1))
