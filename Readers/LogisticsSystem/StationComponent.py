...
from Readers.LogisticsSystem.LocalLogisticOrder import LocalLogisticOrder
from Readers.LogisticsSystem.ShipData import ShipData
from Readers.LogisticsSystem.RemoteLogisticOrder import RemoteLogisticOrder
from Readers.LogisticsSystem.StationStore import StationStore
...
class StationComponent(Reader):
    ...
    unused1 = Int32Field()
    workDroneDatas = ArrayField(lambda: ReaderField(DroneData), length_field='workDroneCount')
    workDroneOrders = ArrayField(lambda: ReaderField(LocalLogisticOrder), length_field='workDroneCount')
    idleShipCount = Int32Field()
    workShipCount = Int32Field()
    idleShipIndices = UInt64Field()
    workShipIndices = UInt64Field()
    unused2 = Int32Field()
    workShipDatas = ArrayField(lambda: ReaderField(ShipData), length_field='workShipCount')
    workShipOrders = ArrayField(lambda: ReaderField(RemoteLogisticOrder), length_field='workShipCount')
    numStorage = ArrayField(lambda: ReaderField(StationStore))
    slots = ConditionalField(lambda: ArrayField(lambda: ReaderField(Slot)), arg_fields='version', condition_func=ge(1))
